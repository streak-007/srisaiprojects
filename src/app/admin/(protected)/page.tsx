...
export default async function AdminDashboardPage() {
  ...
  if (isSupabaseConfigured()) {
    ...
    const [{ count }, { count: reviewCount }] = await Promise.all([
      supabase
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-line bg-white p-5">
          <p className="text-xs font-bold uppercase tracking-wide text-ink-muted">Pending reviews</p>
          <p className="mt-2 font-display text-3xl font-extrabold">{pendingReviews}</p>
          <Link href="/admin/reviews" className="mt-2 inline-block text-sm font-semibold text-teal">
            Moderate →
          </Link>
        </div>
        <div className="rounded-2xl border border-line bg-white p-5">
